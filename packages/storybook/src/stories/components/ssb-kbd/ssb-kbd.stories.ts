...
export const Default = {
  render: ({ label }: Args) => html`<ssb-kbd>${label}</ssb-kbd>`,
} satisfies StoryObj<Args>

export const Combination = {
  render: () =>
    html`<span
      style="display: inline-flex; gap: 0.25rem; align-items: center; font-size: 0.875rem;"
    >
...
export const InText = {
  render: () =>
    html`<p style="font-size: 0.875rem; max-width: 24rem;">
      Press <ssb-kbd>&#8984;</ssb-kbd> <ssb-kbd>K</ssb-kbd> to open the command
      palette and <ssb-kbd>Esc</ssb-kbd> to close it.
    </p>`,
} satisfies StoryObj<Args>
